...
func TestCallEffects_Multiple(t *testing.T) {
	...
	count := Dispatch(plinko.BeforeTransition, effects, payload, trInfo)

	assert.Equal(t, 3, callCount)
	assert.Equal(t, 3, count)

	callCount = 0
	count = Dispatch(plinko.AfterTransition, effects, payload, trInfo)
...
func TestCallSideEffectsWithNilSet(t *testing.T) {
	...
	result := Dispatch(plinko.BeforeTransition, nil, nil, nil)
...
func TestCallEffects(t *testing.T) {
	...
	result := Dispatch(plinko.BeforeTransition, effects, payload, trInfo)
